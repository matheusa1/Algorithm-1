...
void playerCommand(char move[], int position[2], int *chestVerify);
...
int main() {
    ...
    while(boole != 0) {
        ...
        if(strcmp(command, "reset")  == 0) {
            for(int i = 0; i < 1; i++) position[i] = posInit[i];
            chestVerify = 1;
...
void playerCommand(char move[], int position[2], int *chestVerify) {
    ...
    if(((position[0] == 18 && position[1] == 6) || (position[0] == 19 && position[1] == 5) || (position[0] == 19 && position[1] == 7)) && strcmp(move, "open") == 0) {
        *chestVerify = 0;
    }
}